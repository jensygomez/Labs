...
Script Vagrant: |-
  ...
  Vagrant.configure("2") do |config|
    ...
    nodes = [
      ...
      { name: "node02", ip: "192.168.122.12", extra_disks: ['512M'] }, # STG-004: 1 disco de 512MB para LVM
  ...
  HOSTS
        ...
        # ── PROVISIONADO ESPECÍFICO: CONFIGURAR NODE02 COMO SERVIDOR NFS (CON PROBLEMAS INYECTADOS) ──
        if node[:name] == "node02"
          node_config.vm.provision "shell", privileged: true, inline: <<-SHELL
            echo "💾 Configurando node02 como servidor NFS (con problemas inyectados)..."
            ...
            # ─────────────────────────────────────────────────────────────
            # INYECCIÓN DE PROBLEMA #1: LVM mal configurado
            # ─────────────────────────────────────────────────────────────
            ...
            # ─────────────────────────────────────────────────────────────
            # INYECCIÓN DE PROBLEMA #2: /etc/exports MAL CONFIGURADO
            # ─────────────────────────────────────────────────────────────
            # Errores inyectados:
            #   a) IP incorrecta (192.168.122.100 no existe en la red)
            #   b) Solo permisos de lectura (ro) cuando debería ser rw
            #   c) root_squash activo (impide operaciones root desde cliente)
            sudo bash << 'INNEREOF'
            ...
            # ─────────────────────────────────────────────────────────────
            # INYECCIÓN DE PROBLEMA #3: FIREWALL BLOQUEANDO PUERTOS NFS/RPC
            # ─────────────────────────────────────────────────────────────
            sudo bash << 'INNEREOF'
              ...
              ufw allow ssh
              # BLOQUEO EXPLÍCITO de puertos NFS (para simular el problema)
              ufw deny 111/tcp    # rpcbind
              ufw deny 111/udp    # rpcbind
              ufw deny 2049/tcp   # nfs
              ufw deny 2049/udp   # nfs
              ufw --force enable
            INNEREOF
            
            # 3. Reiniciar servicio NFS
            systemctl enable nfs-kernel-server
            systemctl restart nfs-kernel-server
            
            echo "✅ node02 configurado como servidor NFS con 3 problemas inyectados:"
            echo "   [1] LVM: vg_data/lv_shared montado en /srv/shared"
            echo "   [2] /etc/exports: IP incorrecta, solo ro, root_squash activo"
            echo "   [3] Firewall UFW: bloqueando puertos 111 y 2049"
          SHELL
        end
        ...
        if node[:name] == "node03"
          node_config.vm.provision "shell", privileged: true, inline: <<-SHELL
            ...
            # 2. Crear punto de montaje (sin montar aún - el alumno debe hacerlo)
            mkdir -p /mnt/shared
            
            # 3. Intento de montaje inicial que FALLARÁ (por los problemas inyectados en node02)
            echo "⏳ Intentando montar NFS desde node02 (se espera fallo)..."
            mount -t nfs -o rw,bg,soft,timeo=30 192.168.122.12:/srv/shared /mnt/shared 2>/tmp/mount-error.log || \
              echo "⚠️  Montaje fallido (esperado) - ver /tmp/mount-error.log"
            
            echo "✅ node03 configurado como cliente NFS con montaje fallido."
          SHELL
        end
  ...
  # [3/7] node02: Servicio NFS activo
  ...
  # [4/7] node02: Firewall UFW activo
  ...
  # [5/7] node02: Puertos NFS bloqueados (problema inyectado)
  echo -e "\${YELLOW}[5/7] node02: Puertos NFS bloqueados (problema inyectado)\${RESET}"
  if sshpass -p \$PASS ssh \$SSH_OPTS bob@node02 "sudo ufw status | grep -E '2049|111' | grep -q 'DENY'"; then
    ...
    echo -e "      \${RED}✗ Puertos no bloqueados (problema no inyectado)\${RESET}"
    FAIL=1
  fi

  # [6/7] node02: /etc/exports con configuración incorrecta
  ...
  if sshpass -p \$PASS ssh \$SSH_OPTS bob@node02 "grep -q '192.168.122.100' /etc/exports"; then
    echo -e "      \${GREEN}✓ IP incorrecta detectada (problema inyectado)\${RESET}"
  else
    echo -e "      \${RED}✗ Configuración de exports no coincide\${RESET}"
    FAIL=1
  fi
