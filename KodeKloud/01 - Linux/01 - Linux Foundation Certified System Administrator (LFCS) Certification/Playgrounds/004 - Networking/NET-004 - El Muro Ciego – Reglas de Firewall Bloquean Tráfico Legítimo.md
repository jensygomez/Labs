  ...
  EOF
        ...
        if node[:name] == "node02"
          node_config.vm.provision "shell", privileged: true, inline: <<-SHELL
            ...
            # 2. Configurar políticas por defecto DROP (El "Muro")
            iptables -P INPUT DROP
            iptables -P FORWARD DROP
            iptables -P OUTPUT ACCEPT

            # 3. Permitir tráfico local (loopback) esencial
            iptables -A INPUT -i lo -j ACCEPT
            iptables -A OUTPUT -o lo -j ACCEPT

            # 4. PERMITIR SALIDA NUEVA (El administrador puede salir, pero nada entra)
            # Esto simula que el servidor puede hacer updates, pero no recibe respuestas
            # a conexiones iniciadas desde fuera si no se define ESTABLISHED.
            # NOTA: Al no poner ESTABLISHED,RELATED aquí, las respuestas a SYN externos
            # se pierden si la política de INPUT es DROP y no hay regla específica.
            
            # 5. ERROR INTENCIONAL: Se permite SSH solo desde una IP específica que NO es la del cliente
            # o simplemente se olvida la regla de estado. Aquí simulamos el olvido de ESTABLISHED.
            # Si node03 intenta conectar, el SYN llega, pero el SYN-ACK de respuesta se genera.
            # El problema real es que si la política es DROP, necesitamos aceptar ESTABLISHED.
            
            # Vamos a simular un escenario donde se intentó ser seguro pero se rompió la stateful inspection.
            # Se añade una regla para permitir ping (ICMP) solo de origen específico falso, 
            # pero se OLVIDA la regla maestra de estado.
            
            iptables -A INPUT -p icmp --icmp-type echo-request -s 192.168.99.99 -j ACCEPT
            
            # Se habilita logging para que vean los descartes (opcional pero útil para debug)
            iptables -A INPUT -j LOG --log-prefix "IPTABLES-DROP: " --log-level 4
            
            # Guardar reglas actuales (incorrectas) para que sean persistentes al inicio
            # Si existe iptables-save, lo usamos. En Ubuntu suele estar con iptables-persistent
