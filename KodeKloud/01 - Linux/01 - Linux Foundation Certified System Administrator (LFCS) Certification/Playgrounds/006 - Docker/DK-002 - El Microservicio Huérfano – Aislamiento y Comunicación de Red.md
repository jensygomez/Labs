...
tags:
  ...
  - Docker
Script Validacion: |-
  cat << 'EOF' > /tmp/validador-docker-lab-02.sh
  ...
  echo -e "\n\e[1;36m================================================================================\e[0m"
  echo -e "\e[1;33m  🕵️  AUDITORÍA DE RED — INC-4013 (DK-002-v1)\e[0m"
  echo -e "\e[1;36m================================================================================\e[0m"

  # 1. Definición de red personalizada en docker-compose.yml (30%)
  echo -e "\n\e[1;37m⏳ [1/4] Verificando declaración de red personalizada en docker-compose.yml...\e[0m"
  # Buscamos que exista la clave 'networks:' a nivel raíz (sin sangría) Y que los servicios la referencien (con sangría)
  ROOT_NETWORKS=$(grep -cE "^networks:" "$LAB_DIR/docker-compose.yml" 2>/dev/null || echo 0)
  SERVICE_NETWORKS=$(grep -cE "^\s{2}networks:" "$LAB_DIR/docker-compose.yml" 2>/dev/null || echo 0)

  if [ "$ROOT_NETWORKS" -gt 0 ] && [ "$SERVICE_NETWORKS" -gt 0 ]; then
    echo -e "\e[1;32m  ✔ [30%] Se ha definido una red personalizada y los servicios la referencian.\e[0m"
    PUNTOS=$((PUNTOS + 30))
  else
    echo -e "\e[1;31m  ❌ [0%] No se detecta una red personalizada configurada en el compose.\e[0m"
    echo -e "       → Diagnóstico: Revisa que 'networks:' esté a nivel raíz y asignada a cada servicio."
  fi

  # 2. Contenedores conectados a la misma red custom (30%)
  echo -e "\n\e[1;37m⏳ [2/4] Verificando conectividad de red a nivel de Docker Daemon...\e[0m"
  # Obtenemos el nombre de la red que NO sea 'bridge' para cada contenedor
  NET_FRONT=$(docker inspect -f '{{range $k, $v := .NetworkSettings.Networks}}{{if ne $k "bridge"}}{{$k}}{{end}}{{end}}' frontend-app 2>/dev/null)
  NET_BACK=$(docker inspect -f '{{range $k, $v := .NetworkSettings.Networks}}{{if ne $k "bridge"}}{{$k}}{{end}}{{end}}' backend-db 2>/dev/null)

  if [ -n "$NET_FRONT" ] && [ -n "$NET_BACK" ] && [ "$NET_FRONT" = "$NET_BACK" ]; then
    echo -e "\e[1;32m  ✔ [30%] Ambos contenedores están conectados a la misma red personalizada ('$NET_FRONT').\e[0m"
    PUNTOS=$((PUNTOS + 30))
  else
    echo -e "\e[1;31m  ❌ [0%] Los contenedores no comparten una red personalizada.\e[0m"
    echo -e "       → Frontend está en: '$NET_FRONT' | Backend está en: '$NET_BACK'"
    echo -e "       → Solución: Asigna ambos servicios a la misma red en docker-compose.yml y ejecuta 'docker compose up -d'."
  fi

  # 3. Resolución DNS funcional (20%)
  echo -e "\n\e[1;37m⏳ [3/4] Verificando resolución DNS interna (Nombre a IP)...\e[0m"
  # Ejecutamos getent hosts dentro del frontend para ver si resuelve 'backend-db'
  DNS_RESULT=$(docker exec frontend-app getent hosts backend-db 2>/dev/null)

  if echo "$DNS_RESULT" | grep -qE "^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+"; then
    RESOLVED_IP=$(echo "$DNS_RESULT" | awk '{print $1}')
    echo -e "\e[1;32m  ✔ [20%] El DNS de Docker resuelve 'backend-db' correctamente a la IP: $RESOLVED_IP\e[0m"
    PUNTOS=$((PUNTOS + 20))
  else
    echo -e "\e[1;31m  ❌ [0%] Fallo de resolución DNS. El nombre 'backend-db' no se encuentra.\e[0m"
    echo -e "       → Diagnóstico: Ejecuta 'docker exec frontend-app getent hosts backend-db' para ver el error."
    echo -e "       → Solución: Asegúrate de que ambos contenedores estén en la misma red custom (Punto 2)."
  fi

  # 4. Resguardo en bóveda (20%)
  echo -e "\n\e[1;37m⏳ [4/4] Auditando custodia de la configuración en la bóveda...\e[0m"
  if [ -f "/opt/backup-vault/docker-compose-net.bak" ]; then
    # Verificamos que el backup tenga la solución (la palabra 'networks:') y no sea el archivo roto original
    if grep -qE "^networks:" "/opt/backup-vault/docker-compose-net.bak" 2>/dev/null; then
      echo -e "\e[1;32m  ✔ [20%] Copia de seguridad del compose corregido custodiada en /opt/backup-vault/.\e[0m"
      PUNTOS=$((PUNTOS + 20))
    else
      echo -e "\e[1;33m  ⚠️  [5%] Archivo presente en la bóveda, pero parece ser la versión rota (sin la sección 'networks:').\e[0m"
      PUNTOS=$((PUNTOS + 5))
    fi
  else
    echo -e "\e[1;31m  ❌ [0%] No se encontró docker-compose-net.bak en /opt/backup-vault/.\e[0m"
    echo -e "       → Solución: cp $LAB_DIR/docker-compose.yml /opt/backup-vault/docker-compose-net.bak"
  fi

  # Resultado Final
  echo -e "\n\e[1;36m================================================================================\e[0m"
  if [ $PUNTOS -ge 100 ]; then
    echo -e "  🎉 CALIFICACIÓN FINAL: \e[1;32m$PUNTOS / 100\e[0m — ¡Excelente! Dominio de redes y DNS en Docker."
  elif [ $PUNTOS -ge 60 ]; then
    echo -e "  ⚠️  CALIFICACIÓN FINAL: \e[1;33m$PUNTOS / 100\e[0m — Parcialmente resuelto. Revisa los puntos ❌."
  else
    echo -e "  ❌ CALIFICACIÓN FINAL: \e[1;31m$PUNTOS / 100\e[0m — La comunicación entre servicios sigue fallando."
  fi
  echo -e "\e[1;36m================================================================================\e[0m\n"
  EOF

  chmod +x /tmp/validador-docker-lab-02.sh
  bash /tmp/validador-docker-lab-02.sh
  # No borramos el validador inmediatamente para que el estudiante pueda inspeccionar la lógica si lo desea
---
[[Laboratorios del LFCS]]
