...
MODULO_PREFIJO:   "STG"
INCIDENTE_NUEVO:  5           # Número del incidente a crear
REFERENCIA_N1:    4           # Número del incidente anterior (para copiar estructura)
NO_SOLAPAR_CON:   null        # Número del incidente siguiente (null si no existe)

# ─── TEMA DEL NUEVO INCIDENTE ────────────────────────────────────
# Dejar vacío para que la IA proponga basándose en los temas existentes.
# O especificar con estos campos:
TEMA_SUGERIDO:
  titulo:      ""   # ej: "El Disco Cifrado – LUKS + dm-crypt"
  dificultad:  ""   # ej: "7/10"
  nivel:       ""   # ej: "L2" o "L3"
  temas_lfcs:  ""   # ej: "Encrypt Storage, LUKS, cryptsetup"
  recursos:    ""   # ej: "1 disco extra de 1GB en node02"
  escenario:   ""   # descripción breve del problema a inyectar
# ─────────────────────────────────────────────────────────────────
```
...
Listo, este es el prompt maestro completo con todos los cambios integrados. Los puntos clave que se agregaron:

1. **Regla 9**: El script de verificación ahora hace pausa con `read` cuando hay fallos
2. **Regla 10**: El hook del `.bashrc` debe usar `bash` (no `source`) para que la pausa funcione
3. **PASO 5.5**: Sistema de reparación rápida con flag `--fix` y script de provisioning separado

Ahora cuando un incidente falle, el script se quedará esperando que presiones Enter, dándote tiempo de copiar el output y pedirle a la IA que lo arregle. Además, podrás usar `bash /tmp/verify-xxx.sh --fix` para reparar sin destruir la VM.