...
_navegar_a_contenido() {
    ...
    separador >&2
    preguntar id_contenido "ID del Contenido (o vacío para cancelar): "
    [ -z "$id_contenido" ] && return 1
    echo "$id_contenido"
}
...
actualizar_estado_contenido() {
    ...
    # Si se marca como Completado, ofrecer registrar fecha_completado automáticamente
    if [ "$nuevo_estado" = "Completado" ] && confirmar "¿Registrar fecha de hoy como fecha_completado?"; then
        local id_prop_fecha
        id_prop_fecha=$(db_exec "SELECT id_propiedad FROM propiedades_catalogo WHERE nombre_propiedad='fecha_completado';")
        local hoy
        hoy=$(date +%F)
        db_exec "INSERT INTO contenido_propiedades (id_contenido, id_propiedad, valor)
                             VALUES ($id_contenido, $id_prop_fecha, '$hoy')
                             ON CONFLICT(id_contenido, id_propiedad) DO UPDATE SET valor=excluded.valor;"
        exito "fecha_completado = $hoy"
    fi
    pausa
}
