...
# Captura: SOLO comandos (no outputs) en archivos lab-XXXX.lab
#
...
echo -e "\n"
print_info "Iniciando captura de sesión (SOLO comandos)..."
...
# Iniciar sesión script que captura SOLO comandos (no outputs)
...
# -c "$SHELL" = ejecuta el shell actual y guarda solo lo que se escribe
script -a -f -q -c "$SHELL" "$NEW_LAB_FILE"
...
print_warning "Puedes forzar sincronización inmediata ejecutando: ${GREEN}~/Labs/Scripts/Git-auto-sync.sh${NC}"
echo -e "\n"
