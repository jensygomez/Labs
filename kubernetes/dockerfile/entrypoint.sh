#!/bin/bash
set -e
SECRETS_FILE="/workspace/secrets/k8s-secrets.yml"
...
key_path = os.path.join(ssh_dir, "id_k8s_control")
...
with open(config_path, "w") as c:
    c.write("Host *\n")
    c.write("  IdentityFile ~/.ssh/id_k8s_control\n")
...
env_file = "/tmp/.k8s_env"
with open(env_file, "w") as e:
    if "proxmox_password" in data:
        e.write(f'export TF_VAR_proxmox_password="{data["proxmox_password"]}"\n')
    e.write(f'export TF_VAR_ssh_private_key="""{data["ssh_private_key"]}"""\n')
...
EOF
  ...
  rm -f /tmp/.secrets_decrypted.yml /tmp/.k8s_env
